...
This plugin exports a skin texture in several optimized formats. This script is designed for the primary (largest) texture. For any secondary (smaller) textures, see the `Export Skin, Mannequin, or 3D Head (Secondary Texture)` script.

### Compatibility
This script will only work correctly if the image has been saved as a .xcf file, and its dimensions should be powers of 2. The texture can be for a skin, mannequin, 3D head, bolton, or power model. The texture should be a primary texture for the model (largest texture).
